...
def Q5_B(pts, intrinsic, folder_path):
    ...
    #print(final_pts[0].shape)
    
    output = [] 
    intrinsic = np.column_stack((intrinsic, [0, 0, 0]))
    ...
    for i, frame in enumerate(final_pts):
        ...
        #print(points_2D_hom)
        points_3D_hom = np.linalg.lstsq(intrinsic, points_2D_hom.T, rcond=None)
        print(points_3D_hom.shape)
        for j, pt in enumerate(frame):
            pt_hom = np.array([pt[0], pt[1], 1])
            point_3D = np.linalg.lstsq(intrinsic, pt_hom, rcond=None)
            print(point_3D)
            normalized_3D = point_3D
            output[i, j, :] = np.array([normalized_3D[0], normalized_3D[1], normalized_3D[2]])
